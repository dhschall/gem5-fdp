...
BranchRecyclingCache::lookup(ThreadID tid, Addr pc, void *&bp_history)
{
    ...
    if (it != branchRecycleCache.end() && !it->second.empty() &&
        ...
        h->snapshotBrpSaysUse = brpSaysUse(tid, pc);
        h->snapshotHasEntry =
            (it != branchRecycleCache.end() && !it->second.empty());
        h->snapshotBRCEntries =
            (it != branchRecycleCache.end() ? it->second.size() : 0);
    ...
    */
    ...
    //     static_cast<TAGE_SC_L_TAGE::BranchInfo *>(tage_bi->tageBranchInfo);
                             ...
                             const StaticInstPtr &inst, Addr target)
{
    ...
    // DEBUGGING DETERMINE WHICH PREDICTOR WAS FAULTY
    const bool predictedWithRecycle = enableRecycling && h->usedRecycle;
    const bool predictedTaken =
        predictedWithRecycle ? h->recycledTaken : h->base_pred;

    if (predictedTaken != h->actualTaken) {
        DPRINTF(
            RecycledEntry,
            "BRC.mispredict pc=%#x predicted=%d actual=%d source=%s "
            "base_pred=%d recycled_pred=%d entry exists in cache=%d brp says "
            "use=%d\n",
            pc, static_cast<int>(predictedTaken), static_cast<int>(taken),
            predictedWithRecycle ? "recycled" : "base",
            static_cast<int>(h->base_pred), static_cast<int>(h->recycledTaken),
            h->snapshotBRCEntries, h->snapshotBrpSaysUse);
    }
    ...
    const bool entryExists =
        (branchRecycleCache.find(pc) != branchRecycleCache.end() &&
         !branchRecycleCache[pc].empty());
    ...
    //Also check if recycling is correct 
    else if (!baselineCorrect && entryExists) {
