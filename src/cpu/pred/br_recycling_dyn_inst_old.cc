#include "cpu/pred/br_recycling_dyn_inst_old.hh"
...
BranchRecyclingCacheDynInstOld::BranchRecyclingCacheDynInstOld(const Params &p)
...
bool
BranchRecyclingCacheDynInstOld::lookup(ThreadID tid, Addr pc, void *&bp_history)
{
    bp_history = nullptr;
    ...
    if (it != dynInstStacks.end()) {
        ...
        if (enableStride && it->second.strideCounterDynAddr < 50) {
            if (!entries.empty()) {
                entries.pop_back();
            }
            return h->base_pred;
...
void
BranchRecyclingCacheDynInstOld::branchPlaceholder(ThreadID tid, Addr pc,
...
void
BranchRecyclingCacheDynInstOld::updateHistories(ThreadID tid, Addr pc,
...
void
BranchRecyclingCacheDynInstOld::update(ThreadID tid, Addr pc, bool taken,
...
void
BranchRecyclingCacheDynInstOld::squash(ThreadID tid, void *&bp_history)
{
    History *h = bp_history ? static_cast<History *>(bp_history) : nullptr;
    if (!h) {
        return;
...
void
BranchRecyclingCacheDynInstOld::regProbeListeners()
{
    ConditionalPredictor::regProbeListeners();

    if (cpu == nullptr) {
        warn("BranchRecyclingCacheDynInstOld: No CPU registered; probes not "
             "connected\n");
        return;
    }
...
void
BranchRecyclingCacheDynInstOld::pushExecutedOutcome(const o3::DynInstPtr &inst)
{
    if (!inst) {
        return;
    }
...
void
BranchRecyclingCacheDynInstOld::notifyExecutedInst(const o3::DynInstPtr &inst)
{
...
void
BranchRecyclingCacheDynInstOld::notifyDependentInst(
...
void
BranchRecyclingCacheDynInstOld::notifySquashedInst(
...
void
BranchRecyclingCacheDynInstOld::dump(const std::string &filename)
{
...
BranchRecyclingCacheDynInstOld::BranchRecyclingCacheDynInstStats::
